  ...
  return (
    <div className="bg-[#F7F7F5] dark:bg-[#111416] min-h-screen">
      ...
      <article className="py-10 sm:py-16 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Featured Image */}
        <div className="relative aspect-16/10 w-full bg-[#EEF0F0] dark:bg-[#202529] border border-[#D9DCDD] dark:border-[#30363A] overflow-hidden mb-10 rounded-xs shadow-xs">
          <Image
            src={article.image}
            alt={article.imageAlt}
            fill
        ...
        <div className="bg-white dark:bg-[#181C1F] border border-[#D9DCDD] dark:border-[#30363A] p-6 sm:p-10 lg:p-12 space-y-8 text-[#171A1C] dark:text-[#F1F3F4] text-sm sm:text-base leading-relaxed rounded-xs shadow-xs">
          <p className="text-base sm:text-lg font-medium text-[#3F4549] dark:text-[#C2C8CC] leading-relaxed border-l-4 border-[#1F4E5F] dark:border-[#6F9AAA] pl-4 py-1.5 italic bg-[#EEF0F0]/60 dark:bg-[#202529] rounded-r-xs">
            {article.excerpt}
          </p>
          ...
          {relatedProducts.length > 0 && (
            <div className="mt-10 p-6 bg-[#171A1C] text-slate-100 border border-[#30363A] space-y-4 rounded-xs shadow-xs">
              <div className="flex items-center gap-2 text-xs font-mono text-[#6F9AAA] uppercase font-semibold">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                {relatedProducts.map((p) => (
                  ...
                  >
                    <div>
                      <div className="text-[11px] font-mono text-[#6F9AAA] font-semibold">
                        {p.category}
                      </div>
        ...
        {otherArticles.length > 0 && (
          <div className="mt-14 pt-10 border-t border-[#D9DCDD] dark:border-[#30363A]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {otherArticles.map((item) => (
                <div
                  key={item.slug}
                  className="bg-white dark:bg-[#181C1F] border border-[#D9DCDD] dark:border-[#30363A] p-5 hover:border-[#B8BEC0] dark:hover:border-[#41494E] transition-colors flex flex-col justify-between rounded-xs shadow-xs"
                >
                  <div>
                    <span className="text-[11px] font-mono text-[#1F4E5F] dark:text-[#6F9AAA] font-semibold">
                      {item.category}
                    </span>
                  ...
                  <Link
                    href={`/blog/${item.slug}`}
                    className="text-xs font-semibold text-[#1F4E5F] dark:text-[#6F9AAA] hover:text-[#173B48] dark:hover:text-[#8AB2C2] mt-4 inline-flex items-center gap-1"
                  >
